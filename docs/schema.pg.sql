-- =============================================================================
-- 「轻生活」数据库 DDL —— PostgreSQL 16 方言（生产环境）
-- 执行： psql "$DATABASE_URL" -f docs/schema.pg.sql
-- 与 docs/SCHEMA.sql（SQLite，MVP）表/约束一一对应，差异见 docs/ARCHITECTURE.md §3.2
-- 差异映射： INTEGER->BIGINT IDENTITY | INTEGER(0/1)->BOOLEAN | TEXT(json)->JSONB
--            TEXT(日期)->DATE | TEXT(时间戳)->TIMESTAMPTZ | strftime->now()
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS user_profiles (
  ...
  dietary_preference     JSONB       NOT NULL DEFAULT '[]'::jsonb,
  conditions             JSONB       NOT NULL DEFAULT '[]'::jsonb,   -- 敏感字段（D3）
...
CREATE TABLE IF NOT EXISTS user_goals (
  ...
  macro_ratio      JSONB       NOT NULL DEFAULT '{"protein":25,"fat":25,"carb":50}'::jsonb,
